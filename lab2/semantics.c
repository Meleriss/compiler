    ...
    if (strcmp(node->Stroka, "dim") == 0) {
        ...
        while (Children != NULL) {
            ...
            if (foundNode == NULL) {
                ...
                hashtab_add(table->hashtab, name, value, baseType, type);
            } else {
        ...
        // struct listnode *foundNode = findInAllTable(table, name);
...
void print_table(struct AST *node) {
    if (node->table != NULL) {
        ...
        for (int i=0; i < HASHTAB_SIZE; i++) {
            ...
            if (hashnode != NULL) {
                printf("id = %s\tbaseType = %d\ttype = %d\n", 
                hashnode->key, hashnode->baseType, hashnode->type);
            }
        }
    }
    struct ListChild* Children = node->ListChildren;
